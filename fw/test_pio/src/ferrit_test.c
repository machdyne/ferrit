...
// -----------------------------------------------------------------------
// Bit-bang SPI (kept as fallback reference)
// -----------------------------------------------------------------------
...
static void spi_write(const uint8_t *buf, int len) {
	pio_spi_write_blocking(&spi, buf, (size_t)len);
}

static void spi_read(uint8_t *buf, int len) {
	pio_spi_read_blocking(&spi, 0x00, buf, (size_t)len);
}

static void spi_write_read(const uint8_t *tx, uint8_t *rx, int len) {
	pio_spi_write_read_blocking(&spi, tx, rx, (size_t)len);
}
...
int main(void) {
	...
	// PIO SPI — works on any GPIOs regardless of hardware SPI peripheral
	// clk_div=2 -> SCK ~7.8 MHz at 125MHz sys_clk
	pio_spi_init(&spi, pio0, 0, F_SCK, F_MOSI, F_MISO, 2.0f);

	// PIO self-test: MISO pulled high, no CS — expect FF FF FF FF
	{
		uint8_t tx[4] = { 0xFF, 0xFF, 0xFF, 0xFF };
		uint8_t rx[4] = { 0 };
		pio_spi_write_read_blocking(&spi, tx, rx, 4);
		printf("PIO self-test (expect FF FF FF FF): %02X %02X %02X %02X\n",
		       rx[0], rx[1], rx[2], rx[3]);
	}
